...
type Command = TransactionOpCode | bytes
...
def read_script_bytecode(reader: IO[bytes]) -> bytes:
    length = varint.deserialize(reader)
    return reader.read(length)


def build_script_bytecode(commands: Iterable[Command]) -> bytes:
    bytecode = bytearray()
...
def dissect_script_bytecode(bytecode: bytes) -> tuple[Command, ...]:
    ...
    reader, total_length = BytesIO(bytecode), len(bytecode)
